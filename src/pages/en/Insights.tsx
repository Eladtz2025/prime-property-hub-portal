...
const InsightCardHorizontal = ({ item, onClick }: { item: Insight; onClick: () => void }) => (
  ...
  >
    ...
    <div className="p-6 md:p-8 flex flex-col justify-center">
      ...
      {item.summary_en && (
        <p className="text-base text-muted-foreground leading-relaxed font-montserrat">
          {item.summary_en}
        </p>
      )}
      ...
      <section className="py-10 md:py-14 lg:py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          {loading ? (
            ...
            <ScrollAnimated>
              <InsightCardHorizontal item={articles[0]} onClick={() => navigate(`/en/insights/${articles[0].id}`)} />
            </ScrollAnimated>
          ) : articles.length > 1 ? (
            <div className={articles.length === 2 ? "grid md:grid-cols-2 gap-8" : "grid md:grid-cols-3 gap-8"}>
              {articles.map((item, index) => (
                <ScrollAnimated key={item.id} delay={index * 150}>
                  <InsightCardVertical item={item} onClick={() => navigate(`/en/insights/${item.id}`)} />
                </ScrollAnimated>
              ))}
            </div>
          ) : (
      ...
      {!loading && guides.length > 0 && (
        <section className="py-10 md:py-14 lg:py-16 bg-muted/30">
          <div className="max-w-6xl mx-auto px-4">
            ...
            {guides.length === 1 ? (
              <ScrollAnimated>
                <InsightCardHorizontal item={guides[0]} onClick={() => navigate(`/en/insights/${guides[0].id}`)} />
              </ScrollAnimated>
            ) : (

      ...
      <div className="bg-[hsl(220,15%,8%)] text-white border-b border-[hsl(38,60%,45%)]/30">
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="pb-8 text-center">
            <h1 className="font-[Playfair_Display] italic font-light text-3xl md:text-4xl tracking-tight">אנשי מקצוע מומלצים</h1>
            <p className="text-xs text-white/40 mt-2 tracking-[0.15em]">רשימת אנשי מקצוע אמינים ומוכחים</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-10">
        {Object.entries(grouped).map(([profession, pros]) => (
          <div key={profession}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="text-lg font-light tracking-wide text-foreground">{profession}</h2>
            ...
            <div className="space-y-3">
              {pros.map(pro => (
                ...
                >
                  <div>
                    <h3 className="font-[Playfair_Display] text-lg text-foreground">{pro.name}</h3>
                    {pro.area && (
                      <span className="text-xs text-muted-foreground/60 font-light">{pro.area}</span>
                    )}
                    {pro.notes && <p className="text-sm text-muted-foreground/70 mt-1.5 leading-relaxed font-light">{pro.notes}</p>}
                  </div>
                  ...
                  {pro.coupon_code && (
                    <div className="flex items-center gap-2 rounded-lg p-3 border border-[hsl(38,60%,45%)]/30">
                      ...
                      <span className="text-xs font-light text-[hsl(38,60%,35%)]">קוד הנחה:</span>
                      <span className="font-mono font-medium text-sm text-[hsl(38,60%,30%)] tracking-wider">{pro.coupon_code}</span>
                      <Button
                        size="icon"
                        variant="ghost"
                        className="h-7 w-7 mr-auto rounded-full hover:bg-[hsl(38,60%,45%)]/10"
        ...
        {professionals.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            <p className="text-lg font-light">אין אנשי מקצוע להצגה כרגע</p>
          </div>
        )}
      </div>

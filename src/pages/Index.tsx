...
 הפסיפס הצבעוני והמגוון המרתק של תל אביב מחכים לכם.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-12">
            {neighborhoods.map(neighborhood => <div key={neighborhood.id} onClick={() => navigate(`/he/neighborhoods/${neighborhood.id}`)} className="group relative aspect-[3/4] overflow-hidden cursor-pointer" role="button" aria-label={neighborhood.name} tabIndex={0} onKeyDown={e => {
            if (e.key === 'Enter' || e.key === ' ') {
              navigate(`/he/neighborhoods/${neighborhood.id}`);
            }
          }}>
                <img src={neighborhood.image} alt={`שכונת ${neighborhood.name} - תמונת מדריך שכונות תל אביב`} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" decoding="async" />

...
const DynamicPresentationExclusivityForm = () => {
  ...
  const [gushHelka, setGushHelka] = useState("");
  ...
  // Pre-fill property data from deck
  useEffect(() => {
    if (deck?.property) {
      const property = deck.property;
      setPropertyAddress(property.address || "");
      setPropertyCity(property.city || "");
      setPropertyFloor(property.floor?.toString() || "");
      setPropertyRooms(property.rooms?.toString() || "");
      setPropertySize(property.property_size?.toString() || "");
    }
  }, [deck]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dbData = {
        ...
        status: 'signed',
        form_data: { propertyCity, marketingMessages: t.marketingMessages, slug },
  ...
  const handleCreateLink = async () => {
    ...
    try {
      ...
      const link = `${window.location.origin}/offer/${slug}/exclusivity?token=${newToken}`;
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-2xl mx-auto pt-12">
          ...
          <div className="text-center mb-6">
            ...
            {propertyAddress && (
              <p className="text-[#f5c242] text-sm mt-1">{propertyAddress}, {propertyCity}</p>
            )}
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-white/10 border border-white/20 text-white text-sm h-9 rounded-md px-3"
                >
                  <option value={30} className="bg-[#6d5a4a]">30 {isRTL ? 'ימים' : 'days'}</option>
                  <option value={60} className="bg-[#6d5a4a]">60 {isRTL ? 'ימים' : 'days'}</option>
                  <option value={90} className="bg-[#6d5a4a]">90 {isRTL ? 'ימים' : 'days'}</option>
                  <option value={120} className="bg-[#6d5a4a]">120 {isRTL ? 'ימים' : 'days'}</option>
                  <option value={180} className="bg-[#6d5a4a]">180 {isRTL ? 'ימים' : 'days'}</option>
                </select>
              </div>
              <div>
              ...
              <div className="flex items-end pb-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-white/80 text-xs">{t.includesVat}</span>
                </label>
              </div>
            </div>
          </Card>
          ...
          {/* Confirmation */}
          <Card className="bg-[#8b7765]/70 backdrop-blur-sm border-[#f5c242]/30 p-4 mb-6">
            <label className="flex items-start gap-3 cursor-pointer">
              <Checkbox 
                checked={confirmTerms}
                onCheckedChange={(checked) => setConfirmTerms(checked === true)}
                className="mt-0.5 border-white/30 data-[state=checked]:bg-[#f5c242] data-[state=checked]:border-[#f5c242]"
              />
              <span className="text-white/90 text-sm leading-relaxed">{t.confirmUnderstanding}</span>
            </label>
          </Card>
          ...
          {generatedLink && (
            <Card className="bg-green-900/50 backdrop-blur-sm border-green-500/30 p-4 mb-4">
              <p className="text-green-300 text-sm mb-2">{t.linkCreated}</p>
              <div className="bg-black/30 rounded p-2 text-xs text-white/80 break-all">
                {generatedLink}
              </div>
            </Card>
          )}
...
export default DynamicPresentationExclusivityForm;

      ...
      {queues?.map(queue => {
        ...
        return (
          <SavedQueueCard
            ...
            nextProp={nextProp}
            cycleInfo={cycleInfo}
            frequencyLabel={getFrequencyLabel(freqDays)}
            filteredProperties={queue.queue_type === 'property_rotation' ? getFilteredProperties(queue) : []}
            previewText={nextProp ? buildPreviewText(queue) : ''}
            isPreviewOpen={previewQueueId === queue.id}
            onTogglePreview={() => setPreviewQueueId(previewQueueId === queue.id ? null : queue.id)}
            onToggleActive={(isActive) => toggleQueue.mutate({ id: queue.id, is_active: isActive })}
            onEdit={() => openEditQueue(queue)}
            onDelete={() => setDeleteConfirm(queue.id)}
          />
        );
      })}

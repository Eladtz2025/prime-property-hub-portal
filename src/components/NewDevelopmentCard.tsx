...
const formLabels = {
  he: {
    ...
    floors: "קומות",
    units: 'יח"ד',
    successMsg: "ניצור איתך קשר בקרוב!",
  ...
  en: {
    ...
    floors: "Floors",
    units: "Units",
    successMsg: "We'll contact you soon!",
...
export const NewDevelopmentCard = ({
    ...
    try {
      ...
      await supabase.functions.invoke("notify-admins", {
        body: {
          ...
          title: language === "he" ? "פנייה חדשה - פרויקט" : "New Project Inquiry",
          message: `${formData.name} - ${name}`,
  ...
  return (
    <div className="group relative aspect-[4/5] perspective-1000" dir={isRtl ? "rtl" : "ltr"}>
      <div
        className={`relative w-full h-full transition-all duration-700 transform-style-3d ${
          isFlipped ? "rotate-y-180" : ""
        }`}
      >
        {/* Front Side - Premium gradient card */}
        <div
          onClick={() => setIsFlipped(true)}
          className="absolute inset-0 backface-hidden cursor-pointer overflow-hidden rounded-lg"
        >
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-6 text-white">
            ...
            <div className="flex-1 flex flex-col justify-center space-y-4">
              <div>
                <h3 className="font-playfair text-2xl md:text-3xl font-semibold tracking-wide leading-tight">
                  {name}
                </h3>
                {/* Gold separator line */}
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                {roomsRange && (
                  <div className="flex items-center gap-2 text-white/80">
                    <Building2 className="h-4 w-4 text-[#c8a45a]" />
                    <span className="font-montserrat text-sm">
                      {roomsRange} {labels.rooms}
                    </span>
                  </div>
                )}
                {sizeRange && (
                  <div className="flex items-center gap-2 text-white/80">
                    <Maximize2 className="h-4 w-4 text-[#c8a45a]" />
                    <span className="font-montserrat text-sm">
                      {sizeRange} {labels.sqm}
                    </span>
                  </div>
                )}
                {buildingFloors && (
                  <div className="flex items-center gap-2 text-white/80">
                    <Layers className="h-4 w-4 text-[#c8a45a]" />
                    <span className="font-montserrat text-sm">
                      {buildingFloors} {labels.floors}
                    </span>
                  </div>
                )}
                {unitsCount && (
              ...
              {activeFeatures.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {activeFeatures.map(({ icon: Icon, label }) => (
                    <div
                      key={label}
                      className="flex items-center gap-1 text-xs bg-white/10 border border-white/20 text-white/80 px-2.5 py-1 rounded-full"
                    >
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 px-1" dir={isRtl ? "rtl" : "ltr"}>
              <Input
                ...
                required
                className={`bg-background/50 ${isRtl ? "text-right" : ""}`}
              />
              <Input
                type="tel"
              ...
              <Textarea
                ...
                className={`bg-background/50 min-h-[80px] ${isRtl ? "text-right" : ""}`}
              />

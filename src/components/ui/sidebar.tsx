  ...
  (
    ...
    const setCollapsed = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // This sets the cookie to keep the sidebar state.
        document.cookie = `${SIDEBAR_COOKIE_NAME}:collapsed=${collapsedState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
      },
      [setCollapsedProp, collapsed]
    )

    // This is the general toggle function.
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex h-full flex-col bg-sidebar text-sidebar-foreground transition-all duration-200 ease-in-out",
          collapsed && collapsible !== "icon" && "w-14",
          !collapsed && "w-60",
          className
        )}
        data-state={state}
  ...
  (
    ...
    const button = asChild && props.children ? (
      React.cloneElement(props.children as React.ReactElement, {
        ...
        className: cn(sidebarMenuButtonVariants({ variant, size }), className, (props.children as React.ReactElement<{ className?: string }>)?.props?.className),
        ref,
      })
    ) : (
      <Comp

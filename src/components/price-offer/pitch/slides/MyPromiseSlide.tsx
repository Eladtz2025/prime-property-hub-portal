  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center px-8 py-24">
      <div className="w-full max-w-4xl">
        ...
        <div className="mb-8 space-y-4">
          {promises.map((promise, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-start gap-4 rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm transition-colors hover:border-emerald-500/30 hover:bg-white/10"
              >
                {/* Check Icon */}
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-500">
                  <Check className="h-5 w-5 text-white" />
                </div>

                {/* Content */}
                <div className="flex-1">
                  <h3 className="mb-1 text-lg font-semibold text-white">{promise.title}</h3>
                  <p className="text-sm leading-relaxed text-white/60">{promise.description}</p>
                </div>

                {/* Side Icon */}
                <IconComponent className="h-6 w-6 flex-shrink-0 text-emerald-400/50" />
              </div>
            );
          })}
        </div>
        ...
        {/* Guarantee Box */}
        <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-orange-500/10 p-6 text-center">
          <Shield className="mx-auto mb-3 h-8 w-8 text-amber-400" />
          <p className="text-lg leading-relaxed text-white/80">{guaranteeText}</p>
        </div>
      </div>
    </div>
  );

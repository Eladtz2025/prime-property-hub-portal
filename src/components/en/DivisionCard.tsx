...
const DivisionCard = ({ title, description, image, features, link, icon = 'building' }: DivisionCardProps) => {
  ...
  return (
    <Link to={link}>
      <div className="group relative aspect-[4/5] overflow-hidden cursor-pointer animate-fade-in" dir="ltr">
        {/* Image */}
        <img
          src={image}
          alt={title}
        ...
        {/* Content */}
        <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8 text-white text-left">
          {/* Icon */}
          ...
          <ul className="space-y-2 mb-6 md:mb-8">
            {features.map((feature, index) => (
              <li key={index} className="flex items-start gap-2 text-xs md:text-sm justify-start">
          ...
          {/* Action */}
          <div className="flex items-center gap-2 text-white font-montserrat text-sm tracking-wide group-hover:gap-3 transition-all justify-start">

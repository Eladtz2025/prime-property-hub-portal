...
import { saleMemorandumTranslations } from './sale-memorandum-translations';
import { format } from 'date-fns';
import { he } from 'date-fns/locale';
import { BUSINESS_INFO } from '@/constants/business';

export interface SaleMemorandumFormData {
  // Buyer details
  client_name: string;
...
export async function generateSaleMemorandumPDF(formData: SaleMemorandumFormData): Promise<jsPDF> {
  const t = saleMemorandumTranslations[formData.language];
  const isRTL = formData.language === 'he';
  const formDate = format(new Date(formData.form_date), 'dd/MM/yyyy', { locale: he });
  const transferDate = formData.transfer_date 
    ? format(new Date(formData.transfer_date), 'dd/MM/yyyy', { locale: he }) 
    : '—';
  ...
  const sectionStyle = `margin-bottom: 14px; padding: 10px 14px; background: #f8f9fa; border-radius: 6px; border-${isRTL ? 'right' : 'left'}: 4px solid #16a34a;`;
  const headerStyle = `font-size: 13px; font-weight: bold; margin: 0 0 8px 0; color: #166534;`;
  ...
  container.innerHTML = `
    <!-- Header -->
    <div style="text-align: center; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 3px solid #16a34a;">
      <h1 style="font-size: 22px; margin: 0; color: #14532d; font-weight: bold;">${t.title}</h1>
    ...
    <!-- Buyer Details -->
    <div style="${sectionStyle}">
      <h2 style="${headerStyle}">${t.buyerDetails}</h2>
      <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 6px;">
        <p style="${textStyle}"><strong>${t.buyerName}:</strong> ${formData.client_name}</p>
        <p style="${textStyle}"><strong>${t.buyerId}:</strong> ${formData.client_id_number}</p>
        <p style="${textStyle}"><strong>${t.buyerPhone}:</strong> ${formData.client_phone}</p>
        ${formData.client_email ? `<p style="${textStyle}"><strong>${t.buyerEmail}:</strong> ${formData.client_email}</p>` : ''}
      </div>
    </div>
    ...
    <div style="${sectionStyle}">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">
        <p style="${textStyle}"><strong>${t.salePrice}:</strong> ₪${formData.sale_price}</p>
        ...
        ${formData.payment_terms ? `<p style="${textStyle}"><strong>${t.paymentTerms}:</strong> ${formData.payment_terms}</p>` : ''}
        <p style="${textStyle}"><strong>${t.transferDate}:</strong> ${transferDate}</p>
      </div>
    </div>
    
    <!-- Declaration -->
    <div style="margin-bottom: 14px; padding: 12px 16px; background: #f0fdf4; border-radius: 6px; border: 1px solid #bbf7d0;">
    ...
    <!-- Signatures -->
    <div style="margin-top: 20px; padding-top: 16px; border-top: 3px solid #16a34a;">
      <div style="display: flex; justify-content: space-around; gap: 40px;">
        <div style="text-align: center; flex: 1; padding: 12px; background: #f8fafc; border-radius: 8px;">
          <p style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: #166534;">${t.agentSignature}</p>
        ...
        <div style="text-align: center; flex: 1; padding: 12px; background: #f8fafc; border-radius: 8px;">
          <p style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: #166534;">${t.buyerSignature}</p>
...
export async function downloadSaleMemorandumPDF(formData: SaleMemorandumFormData): Promise<void> {
  const pdf = await generateSaleMemorandumPDF(formData);
  const formDate = format(new Date(formData.form_date), 'yyyy-MM-dd');
  const fileName = `sale-memorandum-${formData.client_name.replace(/\s+/g, '-')}-${formDate}.pdf`;
  pdf.save(fileName);
}

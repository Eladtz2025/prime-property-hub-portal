...
serve(async (req) => {
  ...
  const maxPages = settings.scraping.homeless_pages || HOMELESS_CONFIG.MAX_PAGES;
  
  console.log(`🟣 scout-homeless: Starting with ${maxPages} pages max`);
  ...
  try {
    ...
    for (const config of configs) {
      console.log(`Processing Homeless config: ${config.name}`);
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        const url = urls[0];
        console.log(`🟣 Homeless: Scraping page ${page}/${maxPages}: ${url}`);

        // Add delay between pages (not for first page)
        if (page > 1) {
          const delay = HOMELESS_CONFIG.PAGE_DELAY_MS + Math.random() * 1000;

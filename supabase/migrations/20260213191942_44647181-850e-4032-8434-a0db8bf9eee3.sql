...
-- Fix detect_duplicates_batch to require matching floors (prevent false positives for different apartments in same building)
...
BEGIN
  ...
  FOR v_prop IN
    SELECT id, address, city, rooms, size, floor, property_type
  ...
  LOOP
    ...
    WHERE sp.id != v_prop.id
      ...
      -- NEW: Require matching floors to prevent false positives
      AND (
        v_prop.floor IS NULL OR sp.floor IS NULL
        OR sp.floor = v_prop.floor
      )
    LIMIT 1;

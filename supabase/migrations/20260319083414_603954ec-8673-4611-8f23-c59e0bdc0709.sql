-- Fix detect_duplicates_batch: require house number + use normalized address
...
BEGIN
  -- Skip properties missing required fields OR address without a house number
  ...
  LOOP
    ...
    v_normalized_addr := public.normalize_address_for_matching(v_prop.address);
    ...
    WHERE sp.id != v_prop.id
      ...
      AND sp.address ~ '\d'
      AND public.normalize_address_for_matching(sp.address) = v_normalized_addr
...
-- Fix find_property_duplicate: require house number in address
CREATE OR REPLACE FUNCTION public.find_property_duplicate(p_address text, p_city text, p_rooms numeric, p_floor integer, p_size integer DEFAULT NULL::integer, p_property_type text DEFAULT NULL::text, p_exclude_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  -- Require house number in address
  IF p_address !~ '\d' THEN RETURN; END IF;
